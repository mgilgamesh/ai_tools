...
if __name__ == "__main__":
    ...
    eval_data = df_norm.sample(n=evl_data_len)
    train_data = df_norm.sample(n=train_data_len)
    ...
    for i in range(0, train_data.shape[0] - 120 + 1):
        ...
        last_val = pd.Series([sequence.iloc[60, -1]] * 60)
    ...
    while True:
        ...
        for step, (batch_x, batch_y) in enumerate(train_loader):  # 每一步loader释放一小批数据用来学习
            ...
            batch_x_new = batch_x.permute(2, 0, 1)
        ...
        print("均方误差:", np.mean(np.square(output_loss.detach().numpy() - batch_y_new.detach().numpy())))
        epoch += 1
