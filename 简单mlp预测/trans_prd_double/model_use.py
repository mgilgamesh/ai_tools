...
class All_Transformer(nn.Module):
    def __init__(self, input_dim_pos, output_dim_pos, input_dim, output_dim, hidden_dim, num_layers, num_heads,
        ...
        self.decoder = nn.Linear(input_dim, output_dim)
        self.out_layer_1 = nn.Linear(5, 256)
        self.out_layer_2 = nn.Linear(256, 256)
        self.out_layer_3 = nn.Linear(256, 1)
    ...
    def forward(self, x):
        ...
        x_left = x[:1, :, :60]
        # 获取最后一列的最后一个元素并复制60次
        last_val = x[-1:, :, 60:61].repeat(1, 1, 60)
        x_left = torch.cat([x_left, last_val], dim=0)
        x_right = x[1:4, :, 60:]
        print("x_left:", x_left.shape)
        print("x_right:", x_right.shape)

        output_x_right = self.transformer_encoder_pos(x_right)
        ...
        output_all = torch.cat((output_x_right, output_x_left), dim=0)
        print("output_all:", output_all.shape)
        output_all = output_all.permute(2, 1, 0)
        output_all = self.out_layer_1(output_all)
        output_all = self.relu(output_all)
        output_all = self.out_layer_2(output_all)
        output_all = self.relu(output_all)
        output_all = self.out_layer_3(output_all)
        print("output_all:", output_all.shape)
...
if __name__ == "__main__":
    ...
    x_pos = torch.rand(5, batch_size_pos, 120)  # x的shape是(seq_len, batch_size, input_dim)
