...
if __name__ == '__main__':
    ...
    s_smooth = [(x - s_smooth.min()) / (s_smooth.max() - s_smooth.min()) for x in s_smooth]

    crises_smooth = scipy.ndimage.gaussian_filter1d(crise, sigma=sigma1)
    crises_smooth = [(x - crises_smooth.min()) / (crises_smooth.max() - crises_smooth.min()) for x in crises_smooth]
    ...
    while True:
        ...
        for step, (batch_x, batch_y) in enumerate(train_loader):  # 每一步loader释放一小批数据用来学习
            ...
            batch_x_new = batch_x_new[:, :, np.newaxis]
            ...
            loss = criterion(output_loss, batch_y)
            # print("...........")
        ...
        with torch.no_grad():
            for step, (batch_x, batch_y) in enumerate(eval_loader):
                ...
                output_loss = output.permute(1, 0, 2)
                output_loss = output_loss.squeeze()
                loss_in = criterion(output_loss, batch_y)
                sum_loss += loss_in
                eval_loss = loss_in
        ...
        if epoch % 100 == 0 or eval_loss < 1e-3:
            torch.save(predict_net, 'model_pytorch/model_trans_{}.pth'.format(epoch + 1))
